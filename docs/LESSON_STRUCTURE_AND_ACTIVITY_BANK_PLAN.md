...
como dados estáticos versionados, no mesmo padrão de `module-catalog.ts`: literal tipado, sem
...
- `LessonInput.objective` → mantém, mas passa a existir também `aim: string` (intenção da aula,
  opcional; o objective continua sendo o critério observável, e o formulário já pede as duas
...
Homework com PDF/links funcionando com persistência local e testes, seguindo o contrato das outras
